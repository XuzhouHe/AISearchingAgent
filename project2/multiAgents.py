...
def betterEvaluationFunction(currentGameState):
  ...
  curPacStates = currentGameState.getPacmanState()
  ...
  distanceToGhost=[]
  distanceToCapsule=[]
  for ghostState in curGhostStates: ## how away from ghost
      if ghostState.getPosition() == curPacStates.getPosition():
          return float("-inf")
      distanceToGhost.append(manhattanDistance( curPacStates.getPosition(),ghostState.getPosition()))
      #print curPacStates.getPosition()
      if curScaredTimes != 0:
          safeGhostTime = safeGhostTime + sum(curScaredTimes)
  distanceToGhost = min(distanceToGhost)

  for capsule in curCapsules:
      distanceToCapsule.append(manhattanDistance(curPacStates.getPosition(),capsule))
      #print distanceToCapsule
  if distanceToCapsule: distanceToCapsule=min(distanceToCapsule)
  else: distanceToCapsule = float("inf")
  #print distanceToCapsule

  foodLeft_score=1/float(1+curfoodLeft)
  safeTime_score = safeGhostTime/float(1+safeGhostTime) # how many time left for scared ghost
  away_ghost_score = distanceToGhost/float(1+distanceToGhost)
  ready_to_kill_ghost = 1/float(1+distanceToCapsule)

  return currentGameState.getScore() + foodLeft_score + safeTime_score - 2*away_ghost_score + ready_to_kill_ghost
